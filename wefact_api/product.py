...
def product_data_add(code, name, key_phrase, price, cost_center):
    return {"ProductCode": code, "ProductName": name, "ProductKeyPhrase": key_phrase, "PriceExcl": price, "AccountingCostCentre": cost_center}
...
def product_data_edit_from_model(id, line_item: LineItem):
    return product_data_edit(id, line_item.hs_sku, line_item.name, line_item.name, line_item.price, line_item.kostenplaats)
